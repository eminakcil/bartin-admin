...
const createFailToast = (error) => {
  toasts.value.push({
    ...
    content: `Referans eklenmedi. ${error}`,
  })
}
...
const handleSubmit = async () => {
  ...
  if (!vuelidate.value.$invalid) {
    ...
    ReferanceService.create(data.value)
      .then(() => {
        loading.value = false
    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        ...
        <CCardBody>
          ...
          >
            <div class="mb-3">
              <CFormLabel>Referans Bağlantısı</CFormLabel>
              <CFormInput
                type="text"
                ref="firstRef"
                placeholder="Referans bağlantısı giriniz"
                v-model="data.link"
                :invalid="vuelidate.link.$errors.length > 0"
              />
            </div>
            <div class="mb-3">
              <CFormLabel>Referans Görseli</CFormLabel>
              <CFormInput
                type="file"
            ...
            <CButton color="primary">Referans Ekle</CButton>
          </CForm>
        </CCardBody>
      </CCard>
    </CCol>
